...
# Form implementation generated from reading ui file 'proj1_test5.ui'
#
# Created: Mon Feb 27 15:39:39 2017
...
class Ui_resultWindow(QtGui.QWidget):
    ...
    def setupUi(self, resultWindow):
        ...
        resultWindow.resize(314, 390)
        resultWindow.setLayoutDirection(QtCore.Qt.LeftToRight)
        resultWindow.setAutoFillBackground(False)
        resultWindow.setStyleSheet(_fromUtf8("QDialog{background-image: url(:/newPrefix/flatirons.jpg)}\n"
""))
...
"}"))
        ...
        self.sliderObj.setTickPosition(QtGui.QSlider.TicksBothSides)
        self.sliderObj.setTickInterval(5)
        self.sliderObj.setObjectName(_fromUtf8("sliderObj"))
        self.verticalLayout.addWidget(self.sliderObj)
        self.tempValues = QtGui.QLabel(resultWindow)
        sizePolicy = QtGui.QSizePolicy(QtGui.QSizePolicy.Expanding, QtGui.QSizePolicy.Preferred)
        sizePolicy.setHorizontalStretch(0)
        sizePolicy.setVerticalStretch(0)
        sizePolicy.setHeightForWidth(self.tempValues.sizePolicy().hasHeightForWidth())
        self.tempValues.setSizePolicy(sizePolicy)
        font = QtGui.QFont()
        font.setBold(False)
        font.setWeight(50)
        self.tempValues.setFont(font)
        self.tempValues.setObjectName(_fromUtf8("tempValues"))
        self.verticalLayout.addWidget(self.tempValues)
        ...
        QtCore.QObject.connect(self.sliderObj, QtCore.SIGNAL(_fromUtf8("valueChanged(int)")), self.textEditObj.update)
    ...
    def retranslateUi(self, resultWindow):
        ...
        self.tempValues.setText(_translate("resultWindow", "-20        -10          0          10         20         30", None))
    ...
    def printHistory(self):
        ...
        for x in range(0, 9):
            humidity, temperature = Adafruit_DHT.read_retry(sensor, pin)
            myTemp.append(round(temperature,2))
            myHumidity.append(round(humidity,2))
            ts = time.time()
            if graph is 1:
                localtime = datetime.datetime.fromtimestamp(ts).strftime('%M:%S')
        ...
        self.textEditObj.append("Average Temp = "+str(round(sum(myTemp)/len(myTemp),2))+"\n")
        self.textEditObj.append("Average Humidity = "+str(round(sum(myHumidity)/len(myHumidity),2))+"\n")
